...
def initialize_gemini():
    ...
    model = genai.GenerativeModel(
        ...
        system_instruction=(
            ...
            "Sometimes I will also give you a message. If it is not a news article but a short message, just give me more info about it"
        ),
    )
...
def fact_check_page(chat_session):
    st.markdown("<h1 style='text-align: center;'>Fact Check</h1>", unsafe_allow_html=True)
    

    user_input = st.text_area("Enter the statement you want to fact-check")

    if st.button('Check'):
        if user_input:
            prompt = f"Check if this is factually correct and if available show the source: {user_input}"
            ...
            card_content = f"""
                <div style="background-color: white; padding: 20px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); margin-bottom: 20px; color: black;">
                    <h3 style="color: black;">Fact Check Result</h3>
                    <p style="color: black;">{response.text if response else "No response available"}</p>
                </div>
            """
            st.markdown(card_content, unsafe_allow_html=True)

def main_page(chat_session):
